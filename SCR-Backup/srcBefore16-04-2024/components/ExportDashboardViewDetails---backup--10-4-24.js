...
const ExportDashboardViewDetails = ({
  ...
  const handleFIleview = ()=>{
    ...
    .then((res)=>{
      console.log("file responseData", res)
  ...
  // const CCValue = applicationDetail?.copiedResponses?.length ? applicationDetail?.copiedResponses?.map((v,i)=> v.bankName)?.join(', ') : ""
...
{viewShareFile?.length ? viewShareFile?.map((items, index) => {
                  return (
                    <div className="attachemt_form-bx" key={items.id}>
                      <label>
                        {/* {items.filename} */}
