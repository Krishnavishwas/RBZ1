...
{files?.length && files?.find((f) => f.label === items.name)?.file
          ?.name ?  <button type="button" className="remove-file" onClick={() => removeUserImage(items?.name)}>Remove</button> : "" }
        ...
        {otherfiles.map((file, index) => (
          <div key={"other" + (index + 1)} className="attachemt_form-bx">
            ...
            {files?.length && files?.find((f) => f.label === "other" + (index + 1))?.file
