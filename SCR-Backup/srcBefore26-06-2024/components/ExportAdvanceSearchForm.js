...
import ExportAdvanceSearchTable from '../tables/ExportAdvanceSearchTable'
...
const ExportAdvanceSearchForm = () => {
    ...
    const [bankReportResData, setBankReportResData] = useState([]);
...
console.log("roleID >= 4",roleID >= 4);
    ...
    const selectedStatusValue = selectedStatus?.map((res) => res.value)
    ...
    const HandleSubmit = async (e) => {
        ...
        if (validateForm()) {
            ...
            await axios
                .post(APIURL + "ReportData/GetExportReportDataView", {
                    // UserID: UserID.replace(/"/g, ""),
                ...
                .then((res) => {
                    if (res.data.responseCode === "200") {
                        setsubmitbuttonhide(false);
                        // setupdatepopup(true);
                        setBankReportResData(res.data.responseData)
    ...
    useEffect(() => {
        ...
        // newToDate.setDate(newToDate.getDate() - 365);
        // setFromMinDate(newToDate);
        // settoChangeDate(newToDate);
    }, [fromDate]);

    console.log("fromMinDate", fromMinDate);
    ...
    return (
        <>
            <form className="bankReportForm">
                                ...
                                <div className="cccto position-relative">
                                    <div className="multiselect">
                                        ...
                                        {errors?.selectedBank && selectedBank == "" ? (
                                            ...
                                            <>
                                                <span style={{ fontSize: "11px", color: "#8d8c8c", fontStyle: "italic" }}>Max 5 bank select</span>
                                            </>
                ...
                <ExportAdvanceSearchTable bankReportResData={bankReportResData} />
                {/* table data show end */}
                {/* {updatepopup == true ? (
                    <UpdatePopupMessage
                        heading={heading}
...
export default ExportAdvanceSearchForm;
