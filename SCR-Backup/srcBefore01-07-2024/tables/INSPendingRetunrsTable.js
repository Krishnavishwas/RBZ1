...
const INSPendingRetunrsTable = () => {
  ...
  const [INSPendingReturns, setINSPendingReturns] = useState([]);
  ...
  const handleData = async () => {
    await axios
      .post(APIURL + "InspectorateApplication/INSPendingReturn", {
      ...
      .then((res) => {
        if (res.data.responseCode === "200") {
          setINSPendingReturns(res.data.responseData);
        }
      });
  };
  ...
  const GetHandelDetail = async (rbzrefnumber, id) => {
    ...
    await axios
      .post(
        APIURL +
          "InspectorateApplication/GetINSApplicationActionsByApplicationID",
        {
          ID: id,
        }
      )
  ...
  const filteredData = INSPendingReturns?.filter(
    (item) =>
  ...
  return (
    <>
      {INSPendingReturns?.length === 0 ? (
        <div className="p-3">No records to show</div>
      ) : (
        <>
          <div>
            <DataTable
              value={INSPendingReturns}
              scrollable
              scrollHeight="600px"
              className="mt-4"
              paginator={INSPendingReturns.length > 10 ? true : false}
              filters={filters}
                ...
                <div className="login_form ">
                  <h5>
                    <Modal.Header closeButton className="p-0">
                      <Modal.Title>
                        View Inspectorate Pending Return Request --{" "}
                    ...
                    <Modal.Header closeButton className="p-0">
                      <Modal.Title style={{ width: "100%" }}>
                        <div className="row">
                          ...
                          >
                            Edit Inspectorate Pending Return Request --{" "}
                            <big>
...
export default INSPendingRetunrsTable;
