...
const INSCompletedReturnTable = () => {
  const useId = Storage.getItem("userID");
  const rollId = Storage.getItem("roleIDs");
  const bankID = Storage.getItem("bankID");

  const [INSCompletedReturnsData, setINSCompletedReturnsData] = useState([]);
  ...
  //         if (res.data.responseCode === "200") {
  //           setPageLoader(false);
  //           setINSCompletedReturnsData(res.data.responseData);
  ...
  const handleData = async () => {
    setPageLoader(true);
    await axios
      .post(APIURL + "InspectorateApplication/CompletedReturnsINS", {
        UserID: useId.replace(/"/g, ""),
        Status: "125",
        RoleID: rollId,
        DataType: "All",
  ...
  const filteredData = INSCompletedReturnsData?.filter(
    (item) =>
  ...
  useEffect(() => {
    handleData();
    handleTabCount();
    setINSCompletedReturnsData([]);
  }, [tabDepId]);
  console.log("INSCompletedReturnsData", INSCompletedReturnsData);
  return (
    <>
      {/* {tabHeader} */}
      {pageLoader == true ? (
      ...
      ) : INSCompletedReturnsData?.length === 0 ? (
        ...
        <>
          ...
          <div>
            <DataTable
              ...
              value={INSCompletedReturnsData}
              scrollable
              scrollHeight="600px"
              // className="mt-4"
              // paginator={INSCompletedReturnsData.length > 10 ? true : false}
              filters={filters}
...
export default INSCompletedReturnTable;
