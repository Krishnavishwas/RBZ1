...
const FINAllRequetsTable = () => {
  const useId = Storage.getItem("userID");
  const rollId = Storage.getItem("roleIDs");
  const bankID = Storage.getItem("bankID");
  const PdftargetRef = useRef();
  const [FINapproveRequests, setFINapproveRequests] = useState([]); 
  ...
  const handleData = async () => {
    ...
    await axios
      ...
      .then((res) => {
        if (res.data.responseCode === "200") {
          setPageLoader(false);
          setFINapproveRequests(res.data.responseData);
  ...
  const filteredData = FINapproveRequests?.filter(
    (item) =>
  ...
  useEffect(() => {
    ...
    // handlereqData();
    handleTabCount();
    setFINapproveRequests([]);
    // setExportsapproveAllRequests([]);
  }, [tabDepId]);
  
  return (
    <>
    {console.log("FINapproveRequestsFINapproveRequests", FINapproveRequests)}
      {tabHeader}
      {pageLoader == true ? (
...
export default FINAllRequetsTable;
