...
const FINCompletedReturnTable = () => {
  const useId = Storage.getItem("userID");
  const rollId = Storage.getItem("roleIDs");
  const bankID = Storage.getItem("bankID");

  const [FINCompletedReturnsData, setFINCompletedReturnsData] = useState([]);
  ...
  const handleData = async () => {
    setPageLoader(true);
    await axios
      .post(APIURL + "FINApplication/CompletedReturnsFIN", {
        UserID: useId.replace(/"/g, ""),
        Status: "125",
        RoleID: rollId,
        DataType: "All",
      ...
      .then((res) => {
        if (res.data.responseCode === "200") {
          setPageLoader(false);
          setFINCompletedReturnsData(res.data.responseData);
  ...
  const filteredData = FINCompletedReturnsData?.filter(
    (item) =>
  ...
  useEffect(() => {
    handleData();
    handleTabCount();
    setFINCompletedReturnsData([]);
  }, [tabDepId]);
  console.log("FINCompletedReturnsData", FINCompletedReturnsData);
  return (
    <>
      {/* {tabHeader} */}
      {pageLoader == true ? (
      ...
      ) : FINCompletedReturnsData?.length === 0 ? (
        ...
        <>
          ...
          <div>
            <DataTable
              ...
              value={FINCompletedReturnsData}
              scrollable
              scrollHeight="600px"
              // className="mt-4"
              // paginator={FINCompletedReturnsData.length > 10 ? true : false}
              filters={filters}
...
export default FINCompletedReturnTable;
