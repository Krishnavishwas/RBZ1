...
const ImportCompletedReturnTable = () => {
  const useId = Storage.getItem("userID");
  const rollId = Storage.getItem("roleIDs");
  const bankID = Storage.getItem("bankID");

  const [importCompletedReturnsData, setImportCompletedReturnsData] = useState(
    []
  );
  ...
  const handleData = async () => {
    setPageLoader(true);
    await axios
      .post(APIURL + "ImportApplication/CompletedReturnsImport", {
        UserID: useId.replace(/"/g, ""),
        Status: "50",
        RoleID: rollId,
        DataType: tabDepId,
      ...
      .then((res) => {
        if (res.data.responseCode === "200") {
          setPageLoader(false);
          setImportCompletedReturnsData(res.data.responseData);
  ...
  const filteredData = importCompletedReturnsData?.filter(
    (item) =>
  ...
  useEffect(() => {
    handleData();
    handleTabCount();
    setImportCompletedReturnsData([]);
  }, [tabDepId]);
  console.log("importCompletedReturnsData", importCompletedReturnsData);
  return (
    <>
      {/* {tabHeader} */}
      {pageLoader == true ? (
      ...
      ) : importCompletedReturnsData?.length === 0 ? (
        ...
        <>
          ...
          <div>
            <DataTable
              ...
              value={importCompletedReturnsData}
              scrollable
              scrollHeight="600px"
              // className="mt-4"
              // paginator={importCompletedReturnsData.length > 10 ? true : false}
              filters={filters}
...
export default ImportCompletedReturnTable;
