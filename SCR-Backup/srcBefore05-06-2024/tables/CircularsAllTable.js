...
const CircularsAllTable = () => {
      ...
      <>
        ...
        ></i>
        {
          rowData.status == "135" || rowData.status == "125" ? (
            ...
            >
              <i
                class="pi pi-download p-2 nav-link"
  ...
  const releasingDate = (rowData) => {
    return <span>{moment(rowData.releasingDate).format("DD MMM YYYY")}</span>;
  };
  const statusNameData = (rowData) => {
    return <span>{rowData?.statusName ? rowData?.statusName : "_"}</span>;
  };
  ...
  const GetHandelDetail = async (rbzrefnumber, id) => {
    ...
    await axios
      .post(APIURL + "Circular/GetCommentsInfoByRoleIDCircular", {
        CircularID: id,
      })
      .then((res) => {
        if (res.data.responseCode == 200) {
                  ...
                  <table width="100%">
                    ...
                    <tr>
                      ...
                      >
                        ...
                        >
                          {PdfRolename ? PdfRolename?.replace(/"/g, "") : "N/A"}
                        </p>
                        <h3
                          style={{
                            color: "#000",
