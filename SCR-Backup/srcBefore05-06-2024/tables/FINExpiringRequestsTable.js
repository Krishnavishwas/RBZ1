...
const FINExpiringRequestsTable = () => {
  ...
  const [FINsapproveRequests, setFINsapproveRequests] = useState([]);
  ...
  const expiringDate = (rowData) => {
    return <span>{moment(rowData.expiringDate).format("DD MMM YYYY")}</span>;
  };
  ...
  const handleData = async () => {
    await axios
      .post(APIURL + "FINApplication/GetExpiringDataByUserIDFIN", {
        UserID: useId.replace(/"/g, ""),
        RoleID: rollId,
        DataType: tabDepId,
        BankID: bankID,
      ...
      .then((res) => {
        if (res.data.responseCode === "200") {
          setFINsapproveRequests(getCustomers(res.data.responseData));
        }
      });
  };
  ...
  const filteredData = FINsapproveRequests?.filter(
    (item) =>
    ...
    <div className="application-tab w-100 mt-4">
      <ul class="nav nav-pills mb-3">
        <li class="nav-item">
          ...
          >
            All Expiring ({tabCount?.allDataCount})
          </a>
        </li>
        <li class="nav-item">
          <a
          ...
          >
            My Expiring ({tabCount?.myDataCount})
          </a>
        </li>
      </ul>
    </div>
  ...
  return (
    <>
      {tabHeader}
      {FINsapproveRequests?.length === 0 ? (
        <div className="p-3">No records to show</div>
      ) : (
        <>
          <div>
            <DataTable
              value={FINsapproveRequests}
              scrollable
              scrollHeight="500px"
              className="mt-4"
              paginator={FINsapproveRequests.length > 10 ? true : false}
              filters={filters}
...
export default FINExpiringRequestsTable;
