...
const EditorTemplate = ({ bankID, setbanknameEditpdfF, banknameEditpdfF }) => {
    // const editor = useRef(null);
    const navigate = useNavigate();
    // const banktoEditorPage = useLocation();
    // const bankID = banktoEditorPage.state;
    console.log("bankID", bankID);
    // console.log("bankIDPROPS",);
    ...
    const handleBannerimg = (e) => {
        ...
        // setHeaderFileUrl(file)
        const formData = new FormData();
        formData.append('files', file);
        formData.append('bankID', bankID);
        if (file) {
            axios.post(ImageAPI + 'File/UploadFile', formData)
                .then((res) => {
                    setBannerimage(res.data.responseData.filePath);
                    // console.log("res", res.data.responseData.filePath);
                }).catch((error) => {
    ...
    const handleSubmitHtml = async (e) => {
        ...
        const templateIinerPdfHTML = document.querySelector('.template-iiner-pdf').innerHTML;
        // console.log("templateIinerPdfHTML",templateIinerPdfHTML);
        ...
        try {
            ...
            const updateResponse = await fetch(APIURL + 'Admin/UpdateBankLetterHead', {
                method: "POST",
                headers: {
    ...
    return (
        <>
            ...
            {showBankMasterTable ? <BankMasterTable /> :
                    ...
                    <div className="row">
                        <div className="cont-md-12">
                            <div className="card p-4 editor-box">
                                ...
                                <div class="back-btn" onClick={() => {setShowBankMasterTable(true); setbanknameEditpdfF("")}}><i class="bi bi-arrow-left"></i>  </div><h3 className='bank_editname'>{banknameEditpdfF}</h3>
                                </div>
                                </div>
                                <div className="row">
