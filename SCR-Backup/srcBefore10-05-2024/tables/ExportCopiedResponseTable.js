...
const ExportCopiedResponseTable = () => {
  ...
  const [ExportsCopiedResponse, setExportsCopiedResponse] = useState([]);
  ...
  const handleData = async () => {
    await axios
      .post(APIURL + "ExportApplication/GetCopiedResApplicationDataByUserID", {
        UserID: useId.replace(/"/g, ""),
        Status: "10",
        RoleID: rollId,
        DataType: "All",
  ...
  const filteredData = ExportsCopiedResponse?.filter(
    (item) =>
...
export default ExportCopiedResponseTable;
