...
const [othersharefile, setOthersharefile] = useState([]);
  ...
  console.log("userfiles--u", userfiles)
  console.log("otheruserfiles--o", otheruserfiles)
 console.log("attachmentData", attachmentData)
  // console.log("applicationDetail", applicationDetail);
 

  console.log("noDataComment--noDataComment", noDataComment)
...
setSelectedBanks(bankdtata)
  ...
  console.log("applicationdetails--", applicationDetail)
  console.log("masterbank", masterBank)
  console.log("othersharefile", othersharefile)
  ...
  // console.log("IsReturnOption--IsReturnOption", IsReturnOption)
  // console.log("getFrequencyID--getFrequencyID", getFrequencyID)
  ...
  const handleshareFileChange = (e, fileInfoID) => {
    const file = e.target.files[0];
    const index = sharefile?.findIndex((item) => item.fileInfoID === fileInfoID);
    if (index !== -1) {
      setsharefile((prevFiles) => {
        const newFiles = [...prevFiles];
        newFiles[index] = { file, fileInfoID };
        return newFiles;
      });
    } else {
      setsharefile((prevFiles) => [...prevFiles, { file, fileInfoID }]);
    }
  }; 
  ...
  const removeshareImage = (index, fileInfoID) => {
     const updatedShareFile = sharefile.filter(item => item.fileInfoID !== fileInfoID);
  ...
  console.log("IsReturnExpiringDate", IsReturnExpiringDate)
  console.log("getFrequencyID-----Id", getFrequencyID);
  // console.log("IsReturnOption--IsReturnOption", IsReturnOption)
  ...
  const handleOthreuserfile = (e, id) => {
    const otherfile = e.target.files[0];
    setOtherfilesupload([...otherfilesupload, { otherfile, id }]);
  };
  ...
  const HandelSupervisorcheck = () => {
    ...
    setAssignUserID('3C7212C1-53AA-40BA-9321-4C2B53C6587E');
    setSupervisorRoleId("")
  ...
  const validateForm = () => {
    ...
    if (
      ...
      newErrors.assignedTo = "Record Officer is required";
      valid = false;
    }
  ...
  console.log("copyres--", copyresponse)
  // console.log("AssignUserID--AssignUserID", AssignUserID);
          ...
          if (res.data.responseCode === "200") { 
            ...
            .catch((err)=>{
              ...
              axios.post(APIURL + "ExportApplication/CopyingResponses", copyresponse).then((resposnse)=>{
                console.log("res-CopyingResponses", resposnse)
              })
              .catch((error)=>{
      ...
      <img src={items?.filePath} style={{maxHeight:"90px"}} alt="Preview" />
            ...
            {/* -------------start next level------- */}
                ...
                >
                  ...
                  {
              responceCount?.map((item, i)=>{if(item?.id == 2)return(<>{item?.count == 0 ? "" : <span className="counter-tab">{item?.count}</span> }</>)})
            }
                  <span className="btn-collapse">
                    <i className="bi bi-caret-down-fill"></i>
                  </span>
            ...
            {roleID >= 4 ? (
              ...
              responceCount?.map((item, i)=>{if(item?.id == 4)return(<>{item?.count == 0 ? "" : <span className="counter-tab">{item?.count}</span> }</>)})
                ...
                <div className={recordTab ? "customtab" : "d-none"}>
                  {console.log("allcommentallcommentallcommentallcomment", allcomment)}
                        ...
                        >
                          ...
                          <div className="browse-btn">
                            ...
                            <input
                              type="file"
                              onChange={(e) => {
                                handleuserFileChange(e, "other" + (index + 1));
                                handleOthrefile(e, "other" + (index + 1));
                              }}
                            />
                          </div>
                          <span className="filename">
                            {userfiles?.find((f) => f.id === "other" + (index + 1))
...
<div className="col-md-5">
 ...
 {
  sharefile?.length ? <>
  {sharefile?.map((items, index) => {
  return (
    <div
      className="attachemt_form-bx  mt-2"
      key={items.fileInfoID}
    >
      <label
        style={{
          background: "#d9edf7",
      ...
      <span className="filename">
        {sharefile?.find((f) => f.fileInfoID === `sharefile ${index}`)?.file
          ?.name || "No file chosen"}
      </span>

     {sharefile?.length && sharefile.find((f) => f.fileInfoID === `sharefile ${index}`)?.file
...
{othersharefile.map((file, index) => (
  ...
  >
    ...
    <span className="filename">
      {sharefile?.find((f) => f.fileInfoID === "sharefileother" + (index + 1))
        ?.file?.name || "No file chosen"}
    </span>
   {
    sharefile?.length && sharefile?.find((f) => f.fileInfoID === "sharefileother" + (index + 1))
