        ...
        {menuItem?.map((items, index) => {
          return (
            <li className="nav-item" key={index}>
              ...
              <Link className="nav-link collapsed" data-bs-target={"#menu" + index} data-bs-toggle="collapse" to="#"  title={items.menuName} style={{background : "#42a5f5", color :"#fff", margin:"1px 0px"}} onClick={()=>menuindexHanlde(index)}>
                <i className="bi bi-arrow-up-right-square-fill" style={{color :"#fff"}}></i>
                <span>{items.menuName}</span> <i class="bi bi-chevron-down ms-auto" style={{color :"#fff"}}></i>
              </Link>
              <ul  id={"menu" + index} class={menuindex == index ? "nav-content collapse show " :"nav-content collapse"} name={index} data-bs-parent="#sidebar-nav">
                {items?.subMenu?.map((submenuitem, subindex) => { 
                      return (
                        <li
                          key={subindex}
                          className={
                            lastUrlPart === submenuitem.subMenuURL && items.menuName === menuname
                              ? "activemenu"
                              : ""
                          }
                        >
