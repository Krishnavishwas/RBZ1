...
const ExportDashboardEditDetails = ({
  ...
  const removefileImage = (i)=>{
    alert(i)
    const updatedUserFile = files?.filter((item, i) => item?.label != i);
    ...
    setTimeout(() => {
      ...
      doc.html(PdftargetRef.current, {
        ...
        async callback(doc) {          
          addHeader(doc);  
          addWaterMark(doc);        
          doc.setProperties({ title: `${applicationDetail?.rbzReferenceNumber}` });
  ...
  const HandleFileUpload = (e, label, indx, num) => {
    alert(label)
    const file = e.target.files[0];
    ...
    if (index !== -1) {
      setFiles((prevFiles) => {
        const newFiles = [...prevFiles];
        newFiles[index] = { file, label, num };
        return newFiles;
      });
    } else {
      setFiles((prevFiles) => [...prevFiles, { file, label, num }]);
    }
  };
...
console.log("getinfoFile", geninfoFile)
  console.log("files", files)
...
{files?.map((items, index) => {
                            ...
                            >
                              ...
                              <div className="browse-btn">
                                ...
                                <input
                                  type="file"
                                  onChange={(e) =>
                                    HandleFileUpload(e, items.label ? items.label : items.name , index, "1")
                                  }
                                />
                              </div>
                              <span className="filename">
                              {files?.map((f, i) => (
    i === index ? (f?.fileName ? f?.fileName : f.label || "No file chosen") : ""
))}

                              </span>

                              {files?.map((f, i) => (
    i === index && (f?.fileName || f?.label) ? 
  //   <button
  //   type="button"
  //   className="remove-file"
  //   onClick={() =>
  //     removefileImage(items.fileName ? items.fileName : items.label)
  //   }
  // >
  //   Remove
  // </button>
  ...
  <button
  type="button"
  onClick={(e) => handleRemovfile(f.id)}
  className="remove-file"
>
  Remove
</button>
                ...
                <div>
                  {roleID > 5 && recomdAnalyst == "121" ? (
                    <>
                      ...
                      disabled={btnLoader}
                    >
                      {btnLoader ? <span className="loaderwait">Please Wait...</span> : <span>Preview PDF</span>}
                    </button>
                    ...
                    <tr>
                      ...
                      >
                        ...
                        {applicationDetail?.bankAddress3 !=
                          ...
                          : ""}
                        <br />
                        <span style={{ borderBottom: "1px solid #000", fontWeight: "800", fontSize: "19px", }}
                          className="text-uppercase"
                        >
                      ...
                      <td colSpan="2">
                        <table>
                          <tr>
                            ...
                            >
                              <span style={{fontWeight: "800" }}>Response &nbsp;/&nbsp; Conditions</span>
                            </td>
                          </tr>
                          <tr>
                            <td colSpan="2">
