...
const FINApprovedRequestsTable = () => {
  ...
  const handleData = async () => {
    setPageLoader(true);
    await axios
      .post(APIURL + "FINApplication/GetFINDataByUserID", {
        UserID: useId.replace(/"/g, ""),
        Status: "10",
        RoleID: rollId,
        DataType: tabDepId,
      ...
      .then((res) => {
        if (res.data.responseCode === "200") {
          setPageLoader(false);
          setFinsapproveRequests(res.data.responseData);
...
export default FINApprovedRequestsTable;
