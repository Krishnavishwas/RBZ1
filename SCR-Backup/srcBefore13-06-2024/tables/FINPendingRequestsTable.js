...
const FINPendingRequestsTable = () => {
  ...
  const GetHandelDetail = async (rbzrefnumber, id) => {
    ...
    await axios
      .post(
        APIURL + "FINApplication/GetFINNewComments",
        {
          ID: id,
        }
      )
    ...
    await axios
      .post(
        APIURL + "FINApplication/GetFINCommentsInfoByRoleID",
        {
          ApplicationID: id,
        }
      )
  ...
  const tabHeader = (
    <div className="application-tab w-100 mt-4">
      <ul class="nav nav-pills mb-3">
        <li class="nav-item">
          <a class={tabDepId == 'All' ? "nav-link active" : "nav-link"} onClick={(e) => handleClickTag('All')}>All Pending ({tabCount?.allDataCount})</a>
        </li>
        <li class="nav-item">
          <a class={tabDepId == 'My' ? "nav-link active" : "nav-link"} onClick={(e) => handleClickTag('My')}>My Pending ({tabCount?.myDataCount})</a>
        </li>
  ...
  return (
    <>
      ...
      </label>) : ExportsPendingRequests?.length === 0 ? (
...
export default FINPendingRequestsTable;
