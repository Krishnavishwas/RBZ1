...
const ExportDashboardTable = ({tabDepId}) => {
  ...
  const GetReferredData = async (id) => {
    try {
      ...
      await axios
        ...
        .then((res) => {
          console.log("view other department data", res)
          if (res.data.responseCode == 200) {
              ...
              <div className="login_inner">
                ...
                <div className="login_form_panel">
                  ...
                  <div
                          ...
                          {referredData?.sharedFilesData?.length ? referredData?.sharedFilesData?.map((items, index) => {
                    return (
                      <div className="attachemt_form-bx" key={items.id}>
                        <label style={{background: "#d9edf7", padding: "10px"}}>
                          {/* {items.filename} */}
                          {items?.fileName
                            ? items?.fileName
                            : `FileUpload ${index}`}
                  ...
                  : <span style={{fontSize:"14px", display:"block", marginLeft:"10px"}}>No File Found</span>
                }
                            </>
                          )}
                        </div>
