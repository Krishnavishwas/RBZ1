...
const ExportNewRequestForm = () => {
  ...
  const removeUserImage = (id) => { 
    const updatedUserFile = files?.filter(item => item.id !== id);
  ...
  const renderFooter = () => {
    return (
      <div className="flex justify-content-end">
        <button className="validateCrossIcon" onClick={() => setValidateShow(false)}><i class="bi bi-x-circle"></i></button>
      </div>
    );
  };
    ...
    <>
      <form>
        ...
        <div className="inner_form_new ">
          ...
          <label className="controlform">Submit To {roleID == 2 ? "Supervisor" : "Supervisor"}</label>
...
{files?.length && files?.find((f) => f.id === items.id)?.file
          ?.name ?  <button type="button" className="remove-file" onClick={() => removeUserImage(items?.id)}>Remove</button> : "" }
        ...
        {otherfiles.map((file, index) => (
          <div key={"other" + (index + 1)} className="attachemt_form-bx">
            ...
            {files?.length && files?.find((f) => f.id === "other" + (index + 1))?.file
          ?.name ?  <button type="button" className="remove-file" onClick={() => removeUserImage("other" + (index + 1))}>Remove</button> : "" }
