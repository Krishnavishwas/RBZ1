...
<div className="d-flex align-items-center justify-content-between sidebar-header">
        ...
        {isToggled === false ? <div className="toggle-sidebar-btn" onClick={()=>{handleToggle()}}><i className="bi bi-rewind-fill " ></i> </div> : <div className="toggle-sidebar-btn" onClick={()=>{handleToggle()}}><i className="bi bi-fast-forward-fill" ></i></div>}
      </div>
...
{menuItem?.map((items, index) => {
  return (
    <li className="nav-item" key={index}> {/* Assign key to the list item */}
      ...
      <ul className="nav-content">
        {items?.subMenu?.map((submenuitem, subindex) => {
          return (
            <li
              key={subindex}  
              className={lastUrlPart === submenuitem.subMenuURL ? "activemenu" : ""}
            >
              <Link to={"/" + submenuitem.subMenuURL} title={submenuitem.subMenuName} onClick={()=>saveheadername(items.menuName, submenuitem.subMenuName)}>
