  ...
  const columns = [
    {
      ...
      width: "270px",
      cell: (row) => (
        <span title={row.rbzReferenceNumber}>
          {row.rbzReferenceNumber}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      width: "170px",
      cell: (row) => (
        <span title={moment(row.applicationSubmittedDate).format("DD MMM YYYY")}>
    ...
    },
    {
      name: "Status",
      selector: (row) =>
        row.status == 30 ? "Not Approved" : "Submitted to bank Analyst",
      sortable: true,
      cell: (row) => (
        <span title={row.status == 30 ? "Not Approved" : "Submitted to bank Analyst"}>
          { row.status == 30 ? "Not Approved" : "Submitted to bank Analyst"}
        </span>
      ),
      width: "120px",
