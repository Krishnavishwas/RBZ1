  ...
  const SearchableDropdown = ({
    ...
    return (
      <div className="dropdownExpComName">
        ...
        <div
          className={`options ${isOpen && query?.length >= 3 ? "open" : ""}`}
        >
          {query?.length >= 3 && filter(options).length ? (
            ...
            <div style={{ padding: 10, background: "#eeeeee8f" }}>
              No company found
            </div>
          )}
        </div>
        ...
        .post(APIURL + "ExportApplication/CreateExportApplication", {
          ...
          BPNCode:
            registerusertype === "1" && bankID !== ""
              ? exportForm.BPNCode.toUpperCase()
              : "",
          TINNumber:
            registerusertype === "1" && bankID !== ""
