...
const ExportExpiringRequestsTable = () => {
  ...
  //   expiringDate: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.DATE_BEFORE }] }
  // });
  ...
  const renderHeader = () => {
    return (
      <div className="d-flex justify-content-between">
        ...
        <select class="form-select form-select-lg mb-3" aria-label="Default select example" style={{marginLeft: "10rem", fontSize: "18px", fontFamily: "var(--font-family);" }}
        onChange={(e)=>filterNextSevenDays(e.target.value)}
        >
          <option selected disabled>Filter By Days</option>
  ...
  const expiringDate = (rowData) => {
    return (
      <span>
        {moment(rowData.expiringDate).format("DD MMM YYYY")}
      </span>
    );
  };
  ...
  const handleData = async () => {
    await axios
      .post(APIURL + "ExportApplication/GetExpiringDatabyUserID", {
        UserID: useId.replace(/"/g, ""),
        RoleID: rollId,
        DataType: tabDepId,        
        BankID: bankID,
  ...
  const filterNextSevenDays = (value) => {
    if(value!="all"){
      const nextDays = moment().add(value, 'days').format('MM/DD/YYYY');
  ...
  const tabHeader = (
    <div className="application-tab w-100 mt-4">
      <ul class="nav nav-pills mb-3">
        <li class="nav-item">
          <a class={tabDepId == 'All' ? "nav-link active" : "nav-link"} onClick={() => handleClickTag('All')}>All Expiring ({tabCount?.allDataCount})</a>
        </li>
        <li class="nav-item">
          <a class={tabDepId == 'My' ? "nav-link active" : "nav-link"} onClick={() => handleClickTag('My')}>My Expiring ({tabCount?.myDataCount})</a>
        </li>
        {/* <li class="nav-item">
          <a class={tabDepId == 'All' ? "nav-link active" : "nav-link"} onClick={() => handleClickTag('All')}>All Expiring ()</a>
        </li>
        <li class="nav-item">
          <a class={tabDepId == 'My' ? "nav-link active" : "nav-link"} onClick={() => handleClickTag('My')}>My Expiring ()</a>
        </li> */}
...
export default ExportExpiringRequestsTable;