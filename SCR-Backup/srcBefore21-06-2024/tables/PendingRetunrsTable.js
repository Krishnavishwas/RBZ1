...
const PendingRetunrsTable = () => {
  ...
  const handleData = async () => {
    await axios
      .post(APIURL + "ExportApplication/ExportPendingReturn", {
      ...
      .then((res) => {
        if (res.data.responseCode === "200") {
          setExportPendingReturns(res.data.responseData);
        }
      });
  };
  ...
  const filteredData = ExportPendingReturns?.filter(
    (item) =>
  ...
  return (
    <>
      {ExportPendingReturns?.length === 0 ? (
        <div className="p-3">No records to show</div>
      ) : (
        <>
          <div>
            <DataTable
              value={ExportPendingReturns}
              scrollable
              scrollHeight="600px"
              className="mt-4"
              paginator={ExportPendingReturns.length > 10 ? true : false}
              filters={filters}
                      ...
                      <Modal.Title style={{ width: "100%" }}>
                        <div className="row">
                          ...
                          >
                            Edit{" "}
                            {menuname === "Exports" &&
                            submenuname === "Exports Pending Returns"
                              ? "Returns"
                              : ""}{" "}
                            Request --{" "}
                            <big>
...
export default PendingRetunrsTable;
