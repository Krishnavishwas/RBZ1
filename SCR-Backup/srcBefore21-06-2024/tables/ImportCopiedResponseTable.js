...
const ImportCopiedResponseTable = () => {
  ...
  const [ImportsCopiedResponse, setImportsCopiedResponse] = useState([]);
...
console.log("applicationDetail---------",applicationDetail);
  ...
  const handleData = async () => {
    await axios
      .post(APIURL + "ImportApplication/GetCopiedResApplicationDataByUserIDImports", {
        UserID: useId.replace(/"/g, ""),
        Status: "10",
        RoleID: rollId,
        DataType: "All",
      ...
      .then((res) => {
        if (res.data.responseCode === "200") {
            setImportsCopiedResponse(res.data.responseData);
        }
      });
  };
  ...
  const filteredData = ImportsCopiedResponse?.filter(
    (item) =>
  ...
  useEffect(() => {
    handleData();
    handleTabCount();
    setImportsCopiedResponse([]);
  }, []);

  return (
    <>
      {ImportsCopiedResponse?.length === 0 ? (
        <div className="p-3">No records to show</div>
      ) : (
        <>
          <div>
            <DataTable
              value={ImportsCopiedResponse}
              scrollable
              scrollHeight="600px"
              className="mt-4"
              paginator={ImportsCopiedResponse.length > 10 ? true : false}
              filters={filters}
...
export default ImportCopiedResponseTable;
