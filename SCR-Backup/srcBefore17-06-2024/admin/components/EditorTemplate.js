...
const EditorTemplate = ({
  ...
  const handleLogoImgSave = () => {
    ...
    if (typeof logoImgRef.current?.cropper !== "undefined") {
      ...
      toast.success("logo image cropped", {
        autoClose: 2000,
        theme: "colored",
      });
    }
    ...
    if (logoImg) {
      axios
        ...
        .then((res) => {
          console.log("responce --->", res.data.responseData.filePath);
          setCropLogoImg(res.data.responseData.filePath);
          setLogoImg(null);
          toast.success("logo image has been saved");
        })
        .catch((error) => {
  ...
  const handleFooterImgSave = () => {
    ...
    if (typeof footerImgRef.current?.cropper !== "undefined") {
      ...
      toast.success("footer image cropped", {
        autoClose: 2000,
        theme: "colored",
      });
    }
