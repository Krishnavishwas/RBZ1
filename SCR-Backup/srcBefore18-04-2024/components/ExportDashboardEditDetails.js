...
const ExportDashboardEditDetails = ({
                ...
                await axios
                  ...
                  .then(async (response) => {
                    if (response.data.responseCode === "200") {
                      var headerImage =
                        response.data.responseData.headerImageURL;
                      var footerImage =
                        response.data.responseData.footerImageURL;
  ...
  console.log("getinfoFile-vks-", geninfoFile);

  const [newData, setnewData] = useState([]);

  console.log("newData -vks--", newData);
                    ...
                    <div className="attachemt_form-bx" key={items.id}>
                      ...
                      files?.find((f) => f.label === items.name)?.file?.name ? (
                        <button
                          ...
                          onClick={() =>
                            removefileImage(
                              items.fileName ? items.fileName : items.label
                            )
                          }
                        >
                          Remove1
