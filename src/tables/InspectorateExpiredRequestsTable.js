...
const InspectorateExpiredRequestsTable = () => {
  ...
  const handleData = async () => {
    await axios
      .post(APIURL + "InspectorateApplication/GetExpiredDataByUserIDINS", {
        UserID: useId.replace(/"/g, ""),
        RoleID: rollId,
        DataType: tabDepId,
        BankID: bankID,
  ...
  const tabHeader = (
    <div className="application-tab w-100 mt-4">
      <ul className="nav nav-pills mb-3">
        ...
        {/* <li className="nav-item">
          <a className={tabDepId == 'All' ? "nav-link active" : "nav-link"} onClick={() => handleClickTag('All')}>All Expired ()</a>
        </li>
        <li className="nav-item">
          <a className={tabDepId == 'My' ? "nav-link active" : "nav-link"} onClick={() => handleClickTag('My')}>My Expired ()</a>
        </li> */}
      </ul>
    </div>
  );
...
export default InspectorateExpiredRequestsTable;
