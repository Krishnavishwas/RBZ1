...
import FINDashboardRenewEditDetails from "../components/FINDashboardRenewEditDetails";

const FINPendingReturnsTable = () => {
  ...
  const [FINPendingReturns, setFINPendingReturns] = useState([]);
  ...
  const handleData = async () => {
    await axios
      .post(APIURL + "FINApplication/FINPendingReturn", {
      ...
      .then((res) => {
        if (res.data.responseCode === "200") {
            setFINPendingReturns(res.data.responseData);
        }
      });
  };
  ...
  const filteredData = FINPendingReturns?.filter(
    (item) =>
  ...
  return (
    <>
      {FINPendingReturns?.length === 0 ? (
        <div className="p-3">No records to show</div>
      ) : (
        <>
          <div>
            <DataTable
              value={FINPendingReturns}
              scrollable
              scrollHeight="600px"
              className="mt-4"
              paginator={FINPendingReturns.length > 10 ? true : false}
              filters={filters}
...
export default FINPendingReturnsTable;
